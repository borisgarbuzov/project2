...
class GammaSmall(Sample):
    def __init__(self, size, t_par_count, gamma_count, mean, sigma, lag,
        ...
        super().__init__(size=size, t_par_count=t_par_count, mean=mean,
                         sigma=sigma, type_of_noise=type_of_noise)
    ...
    def gamma_hat(self, sample, t):
        ...
        for term_index in range(1, (self.size - self.lag) + 1):
            term = (sample[term_index - 1] * sample[
                (term_index - 1) + self.lag] *
                    norm.pdf(((
                                          term_index / self.size - t) / self.b_cov) / 0.1)) / 0.1
            partial_sum += term
    ...
    def compute(self):
        ...
        for index in range(self.gamma_count):
            ...
            for t_index in range(self.t_par_count):
                ...
                gamma_hat_double_array[t_index, index] = self.gamma_hat(
                    sample=sample, t=self.t_par_array[t_index])
