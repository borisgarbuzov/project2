...
def plot_lrv_hat(precision_dict):
    newey_west_array = list()
    threshold_array = list()
    xs = list()
    for key, value in precision_dict.items():
        xs.append(int(key))
        threshold_array.append(value[0])
        newey_west_array.append(value[1])

    fig = plt.figure()
    plt.xkcd()
    plt.plot(xs, threshold_array, xs, threshold_array, 'ro',  color='green')
    plt.plot(xs, newey_west_array, xs, newey_west_array, 'ro', color='red')
    plt.grid()
    fig.savefig('lol.png')
    plt.show()
...
def compute_and_save_lrv_hat(sample_size_from: int,
    ...
    for sample_size in sample_size_array:
        ...
        for _ in replication_count_array:
